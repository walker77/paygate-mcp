...
export interface WebhookAdminEvent {
  type: 'key.created' | 'key.revoked' | 'key.suspended' | 'key.resumed' | 'key.rotated' | 'key.topup' | 'key.expired' | 'key.credits_transferred' | 'alert.fired' | 'key.auto_topup_configured' | 'key.auto_topped_up' | 'admin_key.created' | 'admin_key.revoked';
  timestamp: string;
  actor: string;
