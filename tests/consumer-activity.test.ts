/**
 * Tests for v8.46.0 — Consumer Activity Heatmap
 *
 * GET /admin/consumer-activity — Shows per-consumer activity metrics
 * including calls, spend, last active time, and activity status.
 */
...
describe('Consumer Activity Heatmap', () => {
  ...
  test('returns complete structure', async () => {
    ...
    const r = await httpGet(port, '/admin/consumer-activity', { 'X-Admin-Key': adminKey });
    ...
    expect(typeof r.body.summary.inactiveConsumers).toBe('number');
  ...
  test('empty when no keys', async () => {
    ...
    expect(r.body.summary.inactiveConsumers).toBe(0);
  });

  test('shows per-consumer activity', async () => {
    ...
    expect(typeof c.lastActive).toBe('string');
  });

  test('classifies active vs inactive consumers', async () => {
    server = makeServer({ defaultCreditsPerCall: 5 });
    const started = await server.start();
    port = started.port;
    adminKey = started.adminKey;
    ...
    await httpPost(port, '/keys', { credits: 100, name: 'inactive-user' }, { 'X-Admin-Key': adminKey });
    ...
    expect(r.body.summary.inactiveConsumers).toBe(1);
    expect(r.body.summary.totalConsumers).toBe(2);
  });

  test('includes credits remaining', async () => {
    server = makeServer({ defaultCreditsPerCall: 10 });
    const started = await server.start();
    port = started.port;
    adminKey = started.adminKey;
    ...
    const c = r.body.consumers[0];
    expect(c.creditsRemaining).toBe(90);
  });
  ...
  test('sorted by totalSpent descending', async () => {
    ...
    const k1 = (await httpPost(port, '/keys', { credits: 200, name: 'small-spender' }, { 'X-Admin-Key': adminKey })).body.key;
    const k2 = (await httpPost(port, '/keys', { credits: 200, name: 'big-spender' }, { 'X-Admin-Key': adminKey })).body.key;
    ...
    expect(r.body.consumers[0].name).toBe('big-spender');
    expect(r.body.consumers[1].name).toBe('small-spender');
  });
  ...
  test('includes status field per consumer', async () => {
    server = makeServer({ defaultCreditsPerCall: 5 });
    const started = await server.start();
    port = started.port;
    adminKey = started.adminKey;
  ...
  test('inactive consumers have null lastActive', async () => {
    server = makeServer({ defaultCreditsPerCall: 5 });
    const started = await server.start();
    port = started.port;
    adminKey = started.adminKey;
  ...
  test('requires admin key', async () => {
    ...
    const r = await httpGet(port, '/admin/consumer-activity');
  ...
  test('rejects POST method', async () => {
    ...
    const r = await httpPost(port, '/admin/consumer-activity', {}, { 'X-Admin-Key': adminKey });
  ...
  test('root listing includes endpoint', async () => {
    ...
    expect(r.body.endpoints.consumerActivity).toBeDefined();
    expect(r.body.endpoints.consumerActivity).toContain('/admin/consumer-activity');
  });
  ...
  test('does not modify system state', async () => {
    ...
    await httpGet(port, '/admin/consumer-activity', { 'X-Admin-Key': adminKey });
    await httpGet(port, '/admin/consumer-activity', { 'X-Admin-Key': adminKey });
