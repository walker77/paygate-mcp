/**
 * Tests for v8.42.0 — Credit Utilization Rate
 *
 * GET /admin/credit-utilization — Shows what percentage of allocated
 * credits are being used, per-key utilization bands, and identifies
 * over/under-provisioned keys.
 */
...
describe('Credit Utilization Rate', () => {
  ...
  test('returns complete structure', async () => {
    ...
    const r = await httpGet(port, '/admin/credit-utilization', { 'X-Admin-Key': adminKey });

    expect(r.status).toBe(200);
    expect(Array.isArray(r.body.bands)).toBe(true);
    ...
    expect(typeof r.body.summary.overallUtilization).toBe('number');
  ...
  test('empty when no keys', async () => {
    ...
    expect(r.body.bands.length).toBe(0);
    expect(r.body.summary.totalAllocated).toBe(0);
    expect(r.body.summary.totalSpent).toBe(0);
    expect(r.body.summary.overallUtilization).toBe(0);
  });

  test('calculates utilization correctly', async () => {
    ...
    await httpPost(port, '/mcp', { jsonrpc: '2.0', id: 4, method: 'tools/call', params: { name: 'tool_a', arguments: {} } }, { 'X-API-Key': k });
    await httpPost(port, '/mcp', { jsonrpc: '2.0', id: 5, method: 'tools/call', params: { name: 'tool_a', arguments: {} } }, { 'X-API-Key': k });

    const r = await httpGet(port, '/admin/credit-utilization', { 'X-Admin-Key': adminKey });

    // totalAllocated = credits remaining + spent = 50 + 50 = 100
    expect(r.body.summary.totalAllocated).toBe(100);
    expect(r.body.summary.totalSpent).toBe(50);
    expect(r.body.summary.overallUtilization).toBe(50);
  });

  test('bands classify utilization levels', async () => {
    server = makeServer({ defaultCreditsPerCall: 5 });
    const started = await server.start();
    port = started.port;
    adminKey = started.adminKey;

    // Key with 0% utilization (no calls)
    await httpPost(port, '/keys', { credits: 100, name: 'unused' }, { 'X-Admin-Key': adminKey });

    // Key with some utilization
    const k2 = (await httpPost(port, '/keys', { credits: 100, name: 'active' }, { 'X-Admin-Key': adminKey })).body.key;
    await httpPost(port, '/mcp', { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'tool_a', arguments: {} } }, { 'X-API-Key': k2 });

    const r = await httpGet(port, '/admin/credit-utilization', { 'X-Admin-Key': adminKey });

    expect(r.body.bands.length).toBeGreaterThanOrEqual(1);
    const band = r.body.bands[0];
    expect(band.range).toBeDefined();
    expect(typeof band.count).toBe('number');
    expect(typeof band.percentage).toBe('number');
  });

  test('0% utilization band for unused keys', async () => {
    server = makeServer({ defaultCreditsPerCall: 5 });
    const started = await server.start();
    port = started.port;
    adminKey = started.adminKey;

    await httpPost(port, '/keys', { credits: 100, name: 'unused1' }, { 'X-Admin-Key': adminKey });
    await httpPost(port, '/keys', { credits: 50, name: 'unused2' }, { 'X-Admin-Key': adminKey });

    const r = await httpGet(port, '/admin/credit-utilization', { 'X-Admin-Key': adminKey });

    const zeroBand = r.body.bands.find((b: any) => b.range === '0%');
    expect(zeroBand).toBeDefined();
    expect(zeroBand.count).toBe(2);
  });

  test('high utilization keys placed correctly', async () => {
    server = makeServer({ defaultCreditsPerCall: 50 });
    const started = await server.start();
    port = started.port;
    adminKey = started.adminKey;

    // Key with 100 credits, spend 50 = 50% utilization
    const k = (await httpPost(port, '/keys', { credits: 100, name: 'heavy' }, { 'X-Admin-Key': adminKey })).body.key;
    await httpPost(port, '/mcp', { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'tool_a', arguments: {} } }, { 'X-API-Key': k });

    const r = await httpGet(port, '/admin/credit-utilization', { 'X-Admin-Key': adminKey });

    const band = r.body.bands.find((b: any) => b.range === '26-50%');
  ...
  test('excludes revoked and suspended keys', async () => {
    ...
    const total = r.body.bands.reduce((s: number, b: any) => s + b.count, 0);
    expect(total).toBe(1);
  });

  test('bands sorted by range ascending', async () => {
    server = makeServer({ defaultCreditsPerCall: 5 });
    const started = await server.start();
    port = started.port;
    adminKey = started.adminKey;
    ...
    const ranges = r.body.bands.map((b: any) => b.range);
  ...
  test('requires admin key', async () => {
    ...
    const r = await httpGet(port, '/admin/credit-utilization');
  ...
  test('rejects POST method', async () => {
    ...
    const r = await httpPost(port, '/admin/credit-utilization', {}, { 'X-Admin-Key': adminKey });
  ...
  test('root listing includes endpoint', async () => {
    ...
    expect(r.body.endpoints.creditUtilization).toBeDefined();
    expect(r.body.endpoints.creditUtilization).toContain('/admin/credit-utilization');
  });
  ...
  test('does not modify system state', async () => {
    ...
    await httpGet(port, '/admin/credit-utilization', { 'X-Admin-Key': adminKey });
    await httpGet(port, '/admin/credit-utilization', { 'X-Admin-Key': adminKey });
