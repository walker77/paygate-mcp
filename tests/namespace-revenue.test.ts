/**
 * Tests for v8.43.0 — Namespace Revenue
 *
 * GET /admin/namespace-revenue — Revenue breakdown by namespace showing
 * which namespaces generate the most credit consumption.
 */
...
describe('Namespace Revenue', () => {
  ...
  test('returns complete structure', async () => {
    ...
    const r = await httpGet(port, '/admin/namespace-revenue', { 'X-Admin-Key': adminKey });
  ...
  test('groups revenue by namespace', async () => {
    server = makeServer({ defaultCreditsPerCall: 10 });
    const started = await server.start();
    port = started.port;
    adminKey = started.adminKey;

    const k1 = (await httpPost(port, '/keys', { credits: 200, name: 'u1', namespace: 'team-alpha' }, { 'X-Admin-Key': adminKey })).body.key;
    const k2 = (await httpPost(port, '/keys', { credits: 200, name: 'u2', namespace: 'team-beta' }, { 'X-Admin-Key': adminKey })).body.key;

    // team-alpha: 2 calls = 20 credits
    ...
    const alpha = r.body.namespaces.find((n: any) => n.namespace === 'team-alpha');
    expect(alpha.totalSpent).toBe(20);
    expect(alpha.keyCount).toBe(1);
    const beta = r.body.namespaces.find((n: any) => n.namespace === 'team-beta');
  ...
  test('keys without namespace grouped as "default"', async () => {
    ...
    const k = (await httpPost(port, '/keys', { credits: 100, name: 'nonamespace' }, { 'X-Admin-Key': adminKey })).body.key;
    await httpPost(port, '/mcp', { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'tool_a', arguments: {} } }, { 'X-API-Key': k });

    const r = await httpGet(port, '/admin/namespace-revenue', { 'X-Admin-Key': adminKey });

    const def = r.body.namespaces.find((n: any) => n.namespace === 'default');
    expect(def).toBeDefined();
    expect(def.totalSpent).toBe(5);
  });
  ...
  test('calculates revenue percentage', async () => {
    ...
    const k1 = (await httpPost(port, '/keys', { credits: 200, name: 'u1', namespace: 'ns1' }, { 'X-Admin-Key': adminKey })).body.key;
    const k2 = (await httpPost(port, '/keys', { credits: 200, name: 'u2', namespace: 'ns2' }, { 'X-Admin-Key': adminKey })).body.key;
    ...
    expect(ns1.percentage).toBe(75);
    const ns2 = r.body.namespaces.find((n: any) => n.namespace === 'ns2');
    expect(ns2.percentage).toBe(25);
  });
  ...
  test('sorted by total spent descending', async () => {
    ...
    const k1 = (await httpPost(port, '/keys', { credits: 200, name: 'u1', namespace: 'small' }, { 'X-Admin-Key': adminKey })).body.key;
    const k2 = (await httpPost(port, '/keys', { credits: 200, name: 'u2', namespace: 'big' }, { 'X-Admin-Key': adminKey })).body.key;
    ...
    expect(r.body.namespaces[0].namespace).toBe('big');
    expect(r.body.namespaces[1].namespace).toBe('small');
  });

  test('includes totalCalls per namespace', async () => {
    server = makeServer({ defaultCreditsPerCall: 5 });
    const started = await server.start();
    port = started.port;
    adminKey = started.adminKey;

    const k = (await httpPost(port, '/keys', { credits: 100, name: 'u1', namespace: 'test-ns' }, { 'X-Admin-Key': adminKey })).body.key;
    await httpPost(port, '/mcp', { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'tool_a', arguments: {} } }, { 'X-API-Key': k });
    await httpPost(port, '/mcp', { jsonrpc: '2.0', id: 2, method: 'tools/call', params: { name: 'tool_b', arguments: {} } }, { 'X-API-Key': k });

    const r = await httpGet(port, '/admin/namespace-revenue', { 'X-Admin-Key': adminKey });

    const ns = r.body.namespaces.find((n: any) => n.namespace === 'test-ns');
    expect(ns.totalCalls).toBe(2);
  });
  ...
  test('excludes revoked and suspended keys', async () => {
    ...
    await httpPost(port, '/keys', { credits: 100, name: 'active', namespace: 'ns2' }, { 'X-Admin-Key': adminKey });

    await httpPost(port, '/keys/revoke', { key: k1 }, { 'X-Admin-Key': adminKey });

    const r = await httpGet(port, '/admin/namespace-revenue', { 'X-Admin-Key': adminKey });

    // Only ns2 should appear
    expect(r.body.namespaces.length).toBe(1);
    expect(r.body.namespaces[0].namespace).toBe('ns2');
  });
  ...
  test('summary includes topNamespace', async () => {
    ...
    const k1 = (await httpPost(port, '/keys', { credits: 200, name: 'u1', namespace: 'winner' }, { 'X-Admin-Key': adminKey })).body.key;
    const k2 = (await httpPost(port, '/keys', { credits: 200, name: 'u2', namespace: 'loser' }, { 'X-Admin-Key': adminKey })).body.key;
    ...
    expect(r.body.summary.topNamespace).toBe('winner');
  ...
  test('requires admin key', async () => {
    ...
    const r = await httpGet(port, '/admin/namespace-revenue');
  ...
  test('rejects POST method', async () => {
    ...
    const r = await httpPost(port, '/admin/namespace-revenue', {}, { 'X-Admin-Key': adminKey });
  ...
  test('root listing includes endpoint', async () => {
    ...
    expect(r.body.endpoints.namespaceRevenue).toBeDefined();
    expect(r.body.endpoints.namespaceRevenue).toContain('/admin/namespace-revenue');
  });
  ...
  test('does not modify system state', async () => {
    ...
    await httpGet(port, '/admin/namespace-revenue', { 'X-Admin-Key': adminKey });
    await httpGet(port, '/admin/namespace-revenue', { 'X-Admin-Key': adminKey });
